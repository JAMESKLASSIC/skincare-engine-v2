...
# Pick product — ONLY from loaded df, no invented names
def pick_product(filtered_df, step_keywords, fallback_text, risk_flag=None):
    step_df = filtered_df[filtered_df['step'].str.contains(step_keywords, case=False, na=False)]

    if step_df.empty:
        # Fallback to any product in filtered_df (never invent)
        step_df = filtered_df
...
# Routine builder — strict on real data only
def build_routine(df, skin_type, concerns, is_sensitive, is_pregnant, using_prescription, area):
    ...
    if concerns:
        ...
        for c in concerns:
            if c == "acne":
                ...
                keywords = "brightening|even tone|fade spots|whitening|hyperpigmentation|dark spots|melasma|pigment|arbutin|kojic|niacinamide|vitamin c|tranexamic|azelaic|licorice"
            elif c == "dryness":
    ...
    routine['Cleanse'], _ = pick_product(filtered, "Gentle cleanser", "Sensitive" if is_sensitive else None)
    routine['Tone'], _ = pick_product(filtered, "Tone|Exfoliate", "Hydrating toner", "Sensitive" if is_sensitive else None)
    routine['Treat'], _ = pick_product(filtered, "Treat", "Targeted serum", "Sensitive" if is_sensitive else None)
    routine['Moisturize'], _ = pick_product(filtered, "Moisturize", "Moisturizer", "Sensitive" if is_sensitive else None)
